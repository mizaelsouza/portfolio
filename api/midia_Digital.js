...
module.exports = (app) => {
            ...
            app.db("midia_digital_produtos")
                ...
                .catch((error) => {
                    if (error.code === "ER_DUP_ENTRY") {
                        res.status(202).json({
                            mensagem:
                                "FALHA ao cadastrar os produtos na midia.",
                        });
                    }
                    ...
                    if (error.code === "ER_NO_REFERENCED_ROW_2") {
                        res.status(202).json({
                            mensagem:
                                "FALHA produto informado não esta cadastrado.",
                        });
                    } else {
                        res.status(500).json({
                            mensagem:
    ...
    const listarMidiaProdutos = async (req, res) => {
        try {
            ...
            res.json({
                mensagem:
                    "Verifique, ocorreu erro ao listarMidiaProdutos os dados",
                motivo: error,
            });
        }
    };
    ...
    const midiaAtualizar = (req, res) => {
        ...
        if (descricao === undefined || descricao === null || descricao === "")
            return res
                .status(202)
                .send({ error: "Descrição - Campo obrigatório" });

        if (fundoUrl === undefined || fundoUrl === null || fundoUrl === "")
            return res
                .status(202)
                .send({ error: "fundoUrl - Campo obrigatório" });

        if (status === undefined || status === null || status === "")
            return res
                .status(202)
                .send({ error: "Status - Campo obrigatório" });
    ...
    const midiaProdutosatualizar = (req, res) => {
        ...
        if (midiaId === undefined || midiaId === null || midiaId === "")
            return res
                .status(202)
                .send({ error: "midiaId - Campo obrigatório" });
