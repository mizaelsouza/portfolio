    ...
    const salvar = async (req, res) => {
        try {
            ...
            if (id) {
                ...
                res.status(500).send(
                    "ERROR: Dados não registrado, verifique o ID.",
                );
            }
        } catch (error) {
            res.status(500).json({
    ...
    const filtrar = (req, res) => {
        ...
        if (descricao) {
            app.db
                ...
                .where("descricao", "rlike", descricao)
                .then((produtos) => {
                    if (produtos) {
                        res.send(produtos);
                    }
        ...
        /*if (ean) {
            app.db.select('*')
                ...
                .catch(err => res.status(404).send('Produto não encontrado.', err))
        }*/
        ...
        if (ean) {
            app.db
                ...
                .catch((err) =>
                    res.json({
                        Menssagem: "Error ao consultar dados pelo EAN",
                        SQL: err.sqlMessage,
                    }),
                );
        }
    ...
    const permissao = (req, res) => {
        ...
        app.db
            ...
            .join(
                "permissao",
                "permissao.id",
                "usuariosxpermissoes.permissaoFK",
            )
